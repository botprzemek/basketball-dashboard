  ...
  <header class="fixed z-50 sm:h-16 md:h-12 lg:h-12">
    <nav class="w-screen grid place-content-center">
      <ul class="w-full flex gap-4">
        <li>
          <NuxtLink :to="localePath('match')" style="text-decoration: none; color: white">
            {{ $t(`path.match`) }}
          </NuxtLink>
        </li>
...
<!--        </li>-->
    ...
    <NuxtLink :to="switchLocalePath(locale.code)" v-for="locale in locales" :key="locale" style="text-decoration: none; color: white">
      {{ locale.name }}
    </NuxtLink>
    <button v-if="token" @click="logout(route.path)">
      Logout
    </button>
    <NuxtLink v-else :to="localePath('admin-login')">
      Login
    </NuxtLink>
  </header>
</template>