export const format = (sec): string => {
    if (!sec) return '00:00'
...
export const game = (socket): void => {
    socket.on("connect_error", (err): void => {
        console.log(`connection error due to ${err.message}`);
    });

    socket.on('initialData', (data): void => {
    ...
    socket.on('updateData', (data): void => {
    ...
    socket.on('updateTimer', (data): void => {
        document.getElementById('gameTime').textContent = format(data.time)
