...
// onMounted(() => {
// admin = io(`${useRuntimeConfig().public.url.ws}/admin`, {
// 	secure: true,
...
// 			.players.filter((player): boolean => player.number === data.number)[0].state =
// 			data.state
// 	})
//
...
// 			.players.filter((player) => player.number === data.number)[0].statistics.seconds =
// 			data.seconds
// 	})
// })
//
...
// 	return teams.value
// 		.filter((team, index) => index === substitution.value.team)[0]
// 		.players.filter((player) => player.state === 'PLAYING')
// }
//
...
// 	return teams.value
// 		.filter((team, index) => index === substitution.value.team)[0]
// 		.players.filter((player) => player.state === 'BENCHED')
// }
//
...
<template>
	...
	<!--				<option-->
	<!--					v-for="(player, index) in getActivePlayers()"-->
	<!--					:value="player.number"-->
	...
	<!--				<option-->
	<!--					v-for="(player, index) in getBenchedPlayers()"-->
	<!--					:value="player.number"-->
	<!--					v-bind:selected="index === 0"-->
	<!--				>-->
	<!--					{{ player.name }} {{ player.lastname }}-->
